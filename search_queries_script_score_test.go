...
func TestScriptScoreQuery(t *testing.T) {
	q := NewScriptScoreQuery(
		NewMatchQuery("message", "opensearchsearch"),
	...
	got := string(data)
	expected := `{"script_score":{"_name":"my_query","boost":5,"min_score":1.1,"query":{"match":{"message":{"query":"opensearchsearch"}}},"script":{"source":"doc['likes'].value / 10"}}}`
	if got != expected {
		t.Errorf("expected\n%s\n,got:\n%s", expected, got)
	}
}
