...
func TestSignificantTermsAggregationWithPartitions(t *testing.T) {
	agg := NewSignificantTermsAggregation().Field("crime_type").Partition(0).NumPartitions(20)
	src, err := agg.Source()
	if err != nil {
		t.Fatal(err)
	}
	...
	got := string(data)
	expected := `{"significant_terms":{"field":"crime_type","include":{"num_partitions":20,"partition":0}}}`
	if got != expected {
		t.Errorf("expected\n%s\n,got:\n%s", expected, got)
	}
}
